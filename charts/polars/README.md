...
| auth.jwks.discoveryUrl | string | `""` | Complete OIDC discovery document URL if it is not served at the conventional `{auth.jwks.issuer}/.well-known/openid-configuration` path. Leave empty to use that conventional path. |
...
| auth.jwks.clientId | string | `""` | OAuth2 client id `pc.ClusterContext` should present when it needs to log in against this OIDC provider (via device authorization). Not secret: this must be a public/native client (no client secret) registered with your OIDC provider. Required when `auth.jwks.enabled` is true, unless `dex.enabled` is true, in which case it defaults to `PolarsOnPrem` (matching the default Dex static client below). |
| auth.jwks.additionalScope | string | `""` | Extra OAuth2 scope(s) `pc.ClusterContext` should request on top of the default `email openid profile`, space-separated if more than one (e.g. `"groups offline_access"`), for instance if your OIDC provider (like Dex) only issues a refresh token when `offline_access` is explicitly requested. |
