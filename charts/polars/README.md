...
$ helm upgrade --install polars polars-inc/polars \
    ...
    --set license.onPrem.workspaceId=<YOUR_WORKSPACE_ID>
...
| runtime.prebuilt.workingDir | string | `""` | Container's working directory. If not specified, the container runtime's default will be used, which might be configured in the container image. Cannot be updated. |
...
| shuffleData.sharedPersistentVolumeClaim | object | `{"create":true,"enabled":false,"existingClaimName":"","size":"125Gi","storageClassName":""}` | Shared persistent storage for shuffle data. |
| shuffleData.sharedPersistentVolumeClaim.enabled | bool | `false` | Enable shared persistent volume claim for shuffle data. |
| shuffleData.sharedPersistentVolumeClaim.create | bool | `true` | Create the PVC resource. Set to false if you want to use an existing PVC. |
...
| auth | object | `{"jwks":{"audience":"","enabled":false,"issuer":"","requiredClaim":"","requiredClaimValues":"","url":""},"oidc":{"audience":"","clientId":"","clientSecret":"","discoveryUrl":"","enabled":false,"requiredClaim":"","requiredClaimValues":"","scope":""}}` | Configure how the scheduler and observatory REST API authenticate incoming requests. This is native to the compute plane (no proxy involved): both endpoints run in the same pod as the `scheduler` container and are validated by the same code path. `auth.jwks` and `auth.oidc` are mutually exclusive; pair either with `dex` below, or with any other OIDC provider that can issue tokens for your own identity backend. |
| auth.jwks | object | `{"audience":"","enabled":false,"issuer":"","requiredClaim":"","requiredClaimValues":"","url":""}` | Validate an OIDC JWT bearer token (via JWKS) before serving a request. Use this when callers already obtain their own access token out of band (there is no compute-plane-mediated login in this mode); to instead have `pc.ClusterContext` log in on its own, use `auth.oidc` below, which also validates incoming tokens the same way. |
...
| auth.jwks.audience | string | `""` | Expected `aud` (audience) claim on incoming tokens. Required when `auth.jwks.enabled` is true, unless `dex.enabled` is true, in which case it defaults to `PolarsOnPrem` (matching the default Dex static client below). If you rename or remove that static client in the Dex config, or a single Dex instance ever fronts more than one cluster, set this explicitly. |
| auth.jwks.requiredClaim | string | `""` | Name of an additional claim that must be present on the token, checked case-sensitively. Leave empty (default) to rely solely on the standard `aud`/`iss`/`exp` checks above. Useful when a single, shared identity provider issues tokens for more than one cluster and `aud` alone isn't enough to scope a token to this one. If `auth.jwks.requiredClaimValues` is left empty, the claim just needs to be present, regardless of its value. |
| auth.jwks.requiredClaimValues | string | `""` | Value `auth.jwks.requiredClaim` must equal (or, for an array claim, contain). Leave empty to only check that `auth.jwks.requiredClaim` is present, regardless of its value. Only meaningful when `auth.jwks.requiredClaim` is set. |
| auth.oidc | object | `{"audience":"","clientId":"","clientSecret":"","discoveryUrl":"","enabled":false,"requiredClaim":"","requiredClaimValues":"","scope":""}` | Have the compute plane itself act as the OIDC client on `pc.ClusterContext`'s behalf: it performs an OAuth2 device authorization grant against your identity provider, and the CLI just polls the compute plane for the outcome, never talking to the identity provider directly. This also validates incoming bearer tokens, the same way `auth.jwks` does, so the two are mutually exclusive. |
| auth.oidc.enabled | bool | `false` | Enable OIDC login and JWT validation on the scheduler and observatory REST API. |
| auth.oidc.discoveryUrl | string | `""` | Complete OIDC discovery document URL (its `/.well-known/openid-configuration` path). Required when `auth.oidc.enabled` is true, unless `dex.enabled` is true and `dex.config.issuer` is set, in which case it defaults to `{dex.config.issuer}/.well-known/openid-configuration`. |
| auth.oidc.clientId | string | `""` | OAuth2 client id the compute plane presents to the identity provider on `pc.ClusterContext`'s behalf. Required when `auth.oidc.enabled` is true, unless `dex.enabled` is true, in which case it defaults to `PolarsOnPrem` (matching the default Dex static client below). |
| auth.oidc.clientSecret | string | `""` | OAuth2 client secret, only needed if your identity provider requires a confidential client. Leave empty for a public client, like the default Dex `PolarsOnPrem` static client below. |
| auth.oidc.audience | string | `""` | Expected `aud` (audience) claim on incoming tokens. Required when `auth.oidc.enabled` is true, unless `dex.enabled` is true, in which case it defaults to `PolarsOnPrem`. If you rename or remove that static client in the Dex config, or a single Dex instance ever fronts more than one cluster, set this explicitly. |
| auth.oidc.scope | string | `""` | Extra OAuth2 scope(s) requested on top of the default `email openid profile`, space-separated if more than one (e.g. `"groups offline_access"`), for instance if your OIDC provider (like Dex) only issues a refresh token when `offline_access` is explicitly requested. |
| auth.oidc.requiredClaim | string | `""` | Name of an additional claim that must be present on the token, checked case-sensitively. Leave empty (default) to rely solely on the standard `aud`/`iss`/`exp` checks above (that Dex's static client `PolarsOnPrem` is dedicated to, for instance, already scopes tokens to this cluster). Useful when a single, shared identity provider fronts more than one cluster. If `auth.oidc.requiredClaimValues` is left empty, the claim just needs to be present, regardless of its value. |
| auth.oidc.requiredClaimValues | string | `""` | Value `auth.oidc.requiredClaim` must equal (or, for an array claim, contain). Leave empty to only check that `auth.oidc.requiredClaim` is present, regardless of its value. Only meaningful when `auth.oidc.requiredClaim` is set. |
| dex | object | `{"config":{"connectors":[],"issuer":"","staticClients":[{"id":"PolarsOnPrem","name":"Polars ClusterContext","public":true}],"storage":{"config":{"file":"/var/dex/dex.db"},"type":"sqlite3"}},"enabled":false,"fullnameOverride":""}` | Deploy Dex (https://dexidp.io) as an OpenID Connect provider. Dex does not authenticate users itself: it federates your own identity backend (LDAP, SAML, GitHub, an upstream OIDC provider, static users, ...) via `dex.config.connectors`, and issues standard OIDC tokens that `auth.jwks`/`auth.oidc` above validate natively. Only `pc.ClusterContext` (script/CLI usage) is expected to authenticate, via the `PolarsOnPrem` static client below (used directly by the compute plane when `auth.oidc` is enabled, or by whatever obtained the token out of band when only `auth.jwks` is enabled). Only `enabled`, `fullnameOverride` and `config` are documented here, but every other value accepted by the `dex` subchart (image, resources, ingress, persistence, ...) can also be set under this key. |
...
| dex.config | object | `{"connectors":[],"issuer":"","staticClients":[{"id":"PolarsOnPrem","name":"Polars ClusterContext","public":true}],"storage":{"config":{"file":"/var/dex/dex.db"},"type":"sqlite3"}}` | Dex application configuration, passed through verbatim to the `dex` subchart. See https://dexidp.io/docs/ for the full schema. `issuer` must be reachable from inside the cluster, for the compute plane to fetch signing keys (unless `auth.jwks.url`/`auth.oidc.discoveryUrl` is set explicitly) and, when `auth.oidc` is enabled, to perform the device-flow login on the CLI's behalf; `pc.ClusterContext` users themselves never need direct network access to it. The default `staticClients` entry (id `PolarsOnPrem`) is required for `pc.ClusterContext`'s built-in login flow to work without extra configuration; only replace it if you are overriding that client. |
